...
parse_app_info_from_snap (pid_t pid, GError **error)
{
  ...
  desktop_file = g_key_file_get_string (app_info->u.snap.keyfile,
                                        SNAP_METADATA_GROUP_INFO,
      ...
      if (g_str_has_suffix (desktop_file, ".desktop"))
        {
          app_info->u.snap.desktop_id =
            g_strndup (desktop_file, strlen (desktop_file) - strlen (".desktop"));
        }
      else
        {
          app_info->u.snap.desktop_id = g_steal_pointer (&desktop_file);
        }
    }
